    ...
    <div className="w-full overflow-hidden relative py-4 md:py-6">
      ...
      <div className="flex w-max animate-marquee">
        {carouselItems.map((logo, index) => (
          <div key={index} className={`flex-shrink-0 w-40 md:w-64 lg:w-80 flex items-center justify-center h-24 md:h-32 lg:h-40 ${logo.alt === 'Mati Materiais' ? 'mx-8 md:mx-16 lg:mx-20' : 'mx-4 md:mx-8'}`}>
            <img 
              src={logo.src} 
              alt={logo.alt} 
              className={`max-w-full max-h-full object-contain transition-all duration-300 opacity-70 hover:opacity-100 ${logo.alt === 'Mati Materiais' ? 'scale-[1.4]' : ''}`}
            />
          </div>
        ))}
      </div>
