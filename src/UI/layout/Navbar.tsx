  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full flex justify-center z-50 pointer-events-none">
        <nav 
          className={`pointer-events-auto bg-white flex items-center justify-between transition-all duration-500 ease-in-out ${
            isScrolled 
              ? "mt-4 w-[95%] md:w-[90%] max-w-[1200px] h-16 shadow-[0_8px_30px_rgb(0,0,0,0.08)] rounded-full px-6 md:px-8 border border-gray-100" 
              : "mt-0 w-full h-20 shadow-none rounded-none px-4 md:px-8 border-b border-gray-100"
          }`}
        >
          {/* Left: Logo */}
        ...
        <div className="hidden lg:flex items-center gap-8 h-full">
          <Link
            to="/"
            className="group relative text-contrast font-medium flex items-center h-full"
          >
            <span>Inicio</span>
          ...
          >
            <div className="flex items-center gap-1 text-contrast font-medium">
              <span>Sobre Nós</span>
              <ChevronDown
                className={`transition-transform duration-300 ${isSobreDropdownOpen ? "rotate-180" : ""}`}
              />
            </div>
            ...
            <div
              ...
              style={{
                backgroundImage: `url(${bgDropdown})`,
            ...
            >
              ...
              <div className="relative z-10 flex flex-col">
                <Link
                  to="/sobre/historia"
                  className="px-5 py-2 text-contrast hover:bg-gray-50/80 hover:text-primary transition-colors"
                >
                  A Nossa Historia
                </Link>
                <Link
                  to="/sobre/recrutamento"
                  className="px-5 py-2 text-contrast hover:bg-gray-50/80 hover:text-primary transition-colors"
                >
                  Recrutamento
                </Link>{" "}
      ...
      {/* Mobile Menu Modal */}
      <div
        className={`fixed inset-0 bg-white z-40 lg:hidden transition-transform duration-300 ease-in-out pt-24 ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="flex flex-col px-6 h-full overflow-y-auto pb-8">
          <Link
            to="/"
            className="text-3xl font-bold text-contrast py-6 border-b border-gray-100"
          >
            Inicio
          </Link>
          ...
          <div className="flex flex-col border-b border-gray-100">
            ...
            >
              <span>Sobre Nós</span>
              <ChevronDown
                className={`w-8 h-8 transition-transform duration-300 ${isSobreDropdownOpen ? "rotate-180" : ""}`}
              />
            </button>
            <div
              className={`flex flex-col gap-4 overflow-hidden transition-all duration-300 ${isSobreDropdownOpen ? "max-h-[300px] pb-6" : "max-h-0"}`}
            >
              <Link
                to="/sobre/historia"
                className="text-xl text-contrast/70 pl-4 py-2"
              >
                A Nossa Historia
              </Link>
              <Link
          ...
          <Link
            to="/contactos"
            className="text-3xl font-bold text-contrast py-6"
          >
            Contactar
          </Link>
